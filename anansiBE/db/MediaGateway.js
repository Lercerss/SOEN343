...
export class MediaGateway {
    ...
    static selectBookCopies(itemIds, callback) {
        ...
        const query = db.format(
            'SELECT MIN(id) as id, book_id FROM book_copies WHERE available = TRUE AND book_id IN (?) GROUP BY book_id',
            [itemIds]
        );
        db.query(query, (err, rows) => {
            if (err) {
    ...
    static selectMovieCopies(itemIds, callback) {
        ...
        const query = db.format(
            'SELECT MIN(id) as id, movie_id FROM movie_copies WHERE available = TRUE AND movie_id IN (?) GROUP BY movie_id',
            [itemIds]
        );
        db.query(query, (err, rows) => {
            if (err) {
    ...
    static selectMusicCopies(itemIds, callback) {
        ...
        const query = db.format(
            'SELECT MIN(id) as id, music_id FROM music_copies WHERE available = TRUE AND music_id IN (?) GROUP BY music_id',
            [itemIds]
        );
        db.query(query, (err, rows) => {
            if (err) {
